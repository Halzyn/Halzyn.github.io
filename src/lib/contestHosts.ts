...
import type {
  ContestGuestHostEmbed,
  ContestModeratorEmbed,
  ContestWithHosts,
  ScheduledContestTeaser,
} from './types'
...
export function contestHostsFromScheduledTeaser(row: ScheduledContestTeaser): ContestHosts | undefined {
  const mods = row.contest_moderators
  const guests = row.contest_guest_hosts
  if ((!mods || mods.length === 0) && (!guests || guests.length === 0)) return undefined
  const h = buildContestHostsFromEmbed({
    id: row.id,
    contest_moderators: mods ?? null,
    contest_guest_hosts: guests ?? null,
  } as ContestWithHosts)
  return h.entries.length > 0 ? h : undefined
}
