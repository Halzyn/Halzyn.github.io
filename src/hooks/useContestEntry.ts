...
function applyDraftToState(
  ...
  return {
    ...
    guesses: mergeDraftIntoGuessesState(draftByTrack, trackList, emptyGuessesForTracks(trackList)),
...
export function useContestEntry({ contest, tracks, slug }: UseContestEntryOptions) {
  ...
  const trackIdsKey = useMemo(() => tracks.map((track) => track.id).join('\0'), [tracks])



  const [name, setName] = useState('')

  const [guesses, setGuesses] = useState<Record<string, string>>(() => emptyGuessesForTracks(tracks))
  ...
  }, [ready, urlEditToken, adminSubmissionId, userId, profileDisplayNameForSubmit])
      ...
      try {
        ...
        if (adminSubmissionId) {
          ...
          if (error) {
            ...
            setPageError(contestEntryLoadErrorMessage(error.message, contest.published))
        ...
        if (editTokenToLoad) {
          ...
          if (error) {
            ...
            if (!urlEditToken && anonymousEditToken) clearStoredContestEntry(slug)
        ...
        if (nextEditToken && !userId) {
          ...
          setSaveNotice(
            ...
            submitEditToken
              ...
              : 'Saved. This device will remember your entry. Bookmark this page to edit from another device.',
          ...
          if (urlEditToken && userId) {
            ...
            navigate(query.length > 0 ? `/contests/${slug}?${query}` : `/contests/${slug}`, {
  ...
  const showClosedToVisitor =
    ...
    closed && ready && !isAdmin && !adminSubmissionId && (!userId || ownerClosedOutcome === 'none')
  ...
  const sessionLocksName = Boolean(userId) && !adminSubmissionId && !useAdminApi && !urlEditToken
  ...
  const missingDisplayNameWhileSignedIn =
    ...
    Boolean(userId) && ready && !profileDisplayNameForSubmit && !urlEditToken && !adminSubmissionId
